...
**헤더 좌측 햄버거(☰) → 좌측 사이드바 드로어(AppSidebar)** 로 목적지를 제공한다. 공간(우리집/재테크/커뮤니티)을 사이드바의 섹션으로 묶고, 각 섹션 안에 해당 공간의 페이지를 나열한다. 항목을 누르면 해당 공간으로 진입하며 그 공간 톤(coral/indigo/violet)이 따라온다. **가계부의 루트(`/inner/cashbook`)가 대시보드(예산 요약)** 를 렌더한다.

> 이전 결정(전역 하단탭)에서 변경됨. 하단탭은 항상 화면 하단을 차지해 모바일에서 부담이 컸다. 네비를 햄버거 뒤로 접어 콘텐츠 영역을 넓히고, 공간별 섹션 구조로 페이지 목록의 확장성을 확보한다. (하단탭 대비 발견성이 낮아지는 트레이드오프가 있으므로, 첫 진입 튜토리얼에서 햄버거 위치를 안내한다.)

### AppShell 구조

```
┌─────────────────────────────────┐
│ ☰  가계부               [+] [⚙]  │  ← 상단: 햄버거 + 페이지명 + 페이지 액션(0~2)
├─────────────────────────────────┤
...
### AppSidebar (헤더 햄버거 → 좌측 드로어)

- 헤더 좌측 햄버거(☰, `PageHeader`의 기본 `leftSlot`)를 인증 + 커플 연결 사용자에게 모든 페이지에서 노출한다(비로그인·온보딩 미완은 AppShell이 렌더되지 않음).
- 드릴다운 페이지가 자체 `leftSlot`(뒤로가기 ← 등)을 넘기면 햄버거 대신 그것을 우선한다.
- 사이드바는 `overlay-kit`로 열고, 항목을 누르거나 바깥/ESC로 닫는다.
- 활성 항목은 현재 경로의 가장 긴 prefix 매칭으로 결정하고, 현재 공간 톤을 따른다.

### 사이드바 항목 구성

| 섹션     | 항목   | 진입 경로          | active 매칭        | 톤     |
| -------- | ------ | ------------------ | ------------------ | ------ |
| 우리집   | 가계부 | `/inner/cashbook`  | `/inner/cashbook`  | coral  |
| 우리집   | 갤러리 | `/inner/photos`    | `/inner/photos`    | coral  |
| 재테크   | 홈     | `/outer`           | `/outer`           | indigo |
| 재테크   | 환테크 | `/outer/forex`     | `/outer/forex`     | indigo |
| 재테크   | 투자   | `/outer/investment`| `/outer/investment`| indigo |
| 재테크   | 적금   | `/outer/savings`   | `/outer/savings`   | indigo |
| 커뮤니티 | 피드   | `/community`       | `/community`       | violet |

- **가계부 루트(`/inner/cashbook`)는 대시보드(예산 요약)** 를 렌더한다. `/inner`로 진입하면 `/inner/cashbook`으로 리다이렉트하고, 로그인 후 랜딩도 `/inner/cashbook`이다.
- 설정·프로필·로그아웃은 **헤더 우측 프로필 아바타 메뉴**에 둔다(사이드바와 분리).
- 가계부 진입(랜딩)=요약(대시보드), 상단탭 4개=내역/캘린더/결산/계획(요약 화면에선 무활성, `CashbookTabs`). 헤더 ⚙ → 단일 설정 페이지(가계부 설정 + 카테고리·알림 링크 통합).
...
- **로고는 단일 유지**. 공간 정체성은 사이드바 섹션·항목과 톤 색으로 표현하고, 브랜드 로고 자체는 분기하지 않는다 (브랜드 일관성).
