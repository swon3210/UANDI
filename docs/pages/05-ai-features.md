...
"점심 김치찌개 9000원, 어제 택시 15000원, 월급 350만원" 같은 자연어 입력,
또는 **영수증 사진 / 카드·계좌 거래 내역 스크린샷**을
**한 번에 여러 건의** 구조화된 가계부 데이터로 변환한다.
항목이 1건이어도 동일하게 배열로 반환되어 UX가 일관된다.

> 거래 내역 목록 스크린샷은 **한 화면 안의 거래 행마다 개별 entry로 추출**한다(영수증 사진처럼 한 장을 1건으로 합치지 않는다).
> 한 달치 사용 내역으로 결산할 수 있도록 한 요청에서 최대 100건까지 추출한다.

### UI 위치

가계부 메인 페이지(`/inner/cashbook/history`) 상단의 **textarea** (여러 줄 입력 허용).
textarea에 📎 버튼으로 영수증/거래내역 스크린샷을 첨부할 수도 있다 (최대 10장).
기존 `+` 버튼(수동 입력)과 병행 사용.
...
항목은 **줄바꿈, 쉼표, '그리고' 등**으로 구분된다. 한 번에 최대 100건까지 허용.
...
```ts
{
  ...
  images?: string[];      // 영수증/거래내역 스크린샷 data URL (최대 10장)
  // text·images 중 최소 하나는 반드시 포함
}
```
...
|------|------|----------|----------|--------------|
| 자연어/영수증 파싱 | GPT-5 mini | ~200 (텍스트) / ~수천 (이미지) | ~100~수천 | $0.001~ |
