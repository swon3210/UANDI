# 페이지 명세: 재테크 — 환테크 (/investment/forex)
...
> | 파일                          | 내용                            | 상태     |
> | ----------------------------- | ------------------------------- | -------- |
> | `06-investment.md`            | 환테크 차트 + 전망 (이 문서)    | 1차 PR   |
> | `06a-investment-forex-trade.md` | 외화 거래 기록 + 보유 + 손익  | Phase 2  |
> | `06b-investment-alerts.md`    | 목표 환율 알림                  | Phase 3  |
...
```
/investment                       → /investment/forex 로 리다이렉트 (자산 대시보드 자리)
/investment/forex                 → 통화 목록 (USD/JPY/EUR/CNY 4개 카드)
/investment/forex/[currency]      → 통화 상세 (차트 + 지표 + AI 전망)
```

- `currency`는 화이트리스트: `USD | JPY | EUR | CNY`. 그 외는 404.
- 진입점은 BottomNav가 아닌 **홈 대시보드의 진입 카드**(`Dashboard.tsx`).
- BottomNav는 기존 3탭(홈/사진/가계부) 유지.
...
- 카드 전체가 `Link` → `/investment/forex`
- 데이터 없을 때(로딩) Skeleton

### 2. 통화 목록 (`/investment/forex`)
...
- **Phase 4** (선택)
  - 다중 통화 비교 차트
  - 적금·예금 영역 (`/investment/savings`) — `/investment` 진입 페이지를 자산 대시보드로 확장
