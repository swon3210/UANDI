...
- 폴더 생성/이름변경/삭제
- 갤러리 메인 페이지 (`/inner/photos`) — 전체/폴더/태그 3개 탭
- 폴더 상세 페이지 (`/inner/photos/folder/[folderId]`) — 기본 그리드만 (필터·선택 모드는 3단계)
- 태그 상세 페이지 (`/inner/photos/tag/[tagName]`) — 기본 그리드만
...
- 사진 클릭 → `/inner/photos/[id]` (2단계에서 상세 페이지 구현)
...
- 폴더 카드 클릭 → `/inner/photos/folder/[folderId]`로 이동
...
- 태그 클릭 → `/inner/photos/tag/[tagName]`으로 이동
...
### 폴더 상세 (/inner/photos/folder/[folderId]) — 기본 모드
...
### 태그 상세 (/inner/photos/tag/[tagName]) — 기본 모드
