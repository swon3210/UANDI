...
- 모든 글 카드: `[⋯]` → `신고하기`. 본인 유저 글이면 `삭제하기` 추가.
...
- **접근 제어 — 보안 핵심**: 권한은 **"라우트를 숨기는 것"이 아니라 서버 측에서 강제**한다.
  - admin 전용 작업(승인 `status='published'`, 숨김 `status='hidden'`, 신고 리셋 등)은 **콜러블 Function 또는 Next API route에서 admin uid를 검증**한 뒤에만 수행한다. 일반 클라이언트는 이 상태 변경을 직접 못 하도록 Firestore 보안 규칙으로 막는다(보안 규칙 표의 "상태 변경은 admin/서버만" 참고).
  - admin UI(`/community/admin`)는 그 서버 엔드포인트를 호출하는 **얇은 껍데기**일 뿐, 화면 노출 차단(비admin → `/community` 리다이렉트)은 편의일 뿐 보안 경계가 아니다.
  - admin uid 목록은 서버 환경변수 또는 `config/admins` 문서로 관리하고, **검증은 반드시 서버에서** 한다.
- 승인 → `status='published'`, `publishedAt = now`. 반려 → `status='hidden'`(또는 삭제).
- 신고됨 → `숨김`은 `status='hidden'`, `유지`는 신고 카운트 리셋.
...
- 클라이언트가 `reportCount`를 직접 증가시키지 않는다(임의 조작 방지).
- `communityPosts/{id}/reports/{uid}` 생성 시 **Function 트리거**가 `reportCount`를 increment하고, 임계치(예: 3) 도달 시 `status='hidden'`으로 자동 전환 + admin 검토 대상으로 둔다. (CLAUDE.md "집계는 요약/카운터" 규칙 준수 — 전체 스캔 금지)
