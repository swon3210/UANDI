...
네비게이션은 **전역 하단탭(데스크톱은 좌측 고정 레일)** 구조다. 일상 목적지(가계부·갤러리·재테크·커뮤니티) 4개를 평면으로 상시 노출하고, 공간(우리집/재테크/커뮤니티) 구분은 네비게이션 레이어가 아니라 **톤(색)으로만** 표현한다. 별도 "홈" 탭은 없고 **가계부 탭 루트(`/inner/cashbook`)가 대시보드**다. 자세한 패턴은 `08-spaces.md` §4 참고.
...
# 우리집 (커플 공동)
/inner                       → /inner/cashbook 으로 리다이렉트 (별도 홈 탭 없음)
...
/inner/cashbook              → 가계부 (루트 = 대시보드: 예산 요약 + 진입점)
