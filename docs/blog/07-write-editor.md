...
한때 다음·네이버·부산대 검사기를 붙였다가 **걷어냈다.** 셋 다 공식 API가 아니라
웹페이지를 긁는 방식이고, 실제로 셋 모두 연결되지 않는 환경이 있었다. 되는 날과
안 되는 날이 갈리는 기능은 글쓰기를 돕기는커녕 의심만 만든다.

다시 붙인다면 스크래핑이 아닌 경로(정식 API나 로컬 사전)를 먼저 확인할 것.
