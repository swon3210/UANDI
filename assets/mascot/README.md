...
| 파일 | 캐릭터 | 해상도 | 비고 |
| --- | --- | --- | --- |
| `mascot-coral-master.png` | 코랄 스카프 | 505×600 RGBA | 우리집/Primary |
| `mascot-sage-master.png` | 세이지(그린) 스카프 | 505×600 RGBA | 짝꿍 |

## 용도

- **이 파일들은 빌드에 포함되지 않는다** (어떤 코드도 import하지 않음). 보관용 아카이브.
- 실제 앱이 쓰는 **최적화본(384px palette)** 은 `packages/ui/src/assets/mascot-loading*.png` 에 있고 `MascotLoader` 가 import한다.
