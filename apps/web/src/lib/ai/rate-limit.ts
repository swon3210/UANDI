 ...
 * 한도 초과 시 false를 반환한다.
 */
export async function checkAndIncrementUsage(coupleId: string): Promise<boolean> {
  const today = dayjs().format('YYYY-MM-DD');
  const docRef = adminDb().collection('couples').doc(coupleId).collection('meta').doc('aiUsage');
