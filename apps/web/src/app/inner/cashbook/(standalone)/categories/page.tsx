...
export default function CashbookCategoriesPage() {
  ...
  const handleDelete = async (category: CashbookCategory) => {
    ...
    const childCount = allCategories.filter((c) => c.parentCategoryId === category.id).length;
    ...
    if (childCount > 0 || usedCount > 0) {
      ...
      if (childCount > 0) {
        messageLines.push(`하위 카테고리 ${childCount}개도 함께 삭제됩니다.`);
      }
      if (usedCount > 0) {
        messageLines.push(
      ...
      <Header
        ...
        rightSlot={
          <Button
            ...
            onClick={() => router.push('/inner/cashbook/categories/orphans')}
          >
            <Inbox size={20} />
          </Button>
        }
