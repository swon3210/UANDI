...
import {
  BulkEditGrid,
  type BulkEditRow,
} from '@/components/cashbook/plan-bulk-edit/BulkEditGrid';
...
const ANNUAL_MAIN = '/cashbook/plan/annual';
...
type SaveArgs = Parameters<
  ReturnType<typeof useBulkUpdatePlanItems>['mutateAsync']
>[0];
...
function BulkEditPageInner({
  ...
  const totalsBefore = useMemo(() => {
    const beforeItems = items.map((it) => ({
      ...
      annualAmount: (originalSnapshot[it.id] ?? it.monthlyAmounts).reduce(
        (s, v) => s + v,
        0
      ),
    }));
  ...
  const totalsAfter = useMemo(
    () => totalsFromItems(itemsForValidation),
    [itemsForValidation]
  );
  ...
  const changedRows = useMemo(
    () =>
      rows.filter((row) =>
        row.monthlyAmounts.some((v, i) => v !== row.baseline[i])
      ),
    [rows]
  );
