...
이 순서를 어디서 밟아봤다. 올해 회사에서 기획자와 디자이너가 쓰는 자동화 워크플로우를 만들 때다. [AI가 짠 코드를 믿지 못했다](/posts/ai-workflow-verification-gates)에 적었던 그 체계다. 만들기보다 쓰이게 만드는 데 공이 들었다. 진입점을 명령 두 개로 줄이고, 단계마다 사람이 승인하는 문을 두고서야 다른 직군이 실제로 썼다. 저자 식으로 말하면 나는 사내에서 배치와 정착을 하고 있었다. 직군 이름을 몰랐을 뿐이다.
